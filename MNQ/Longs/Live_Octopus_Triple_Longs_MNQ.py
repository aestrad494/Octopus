...
class LiveOctopus(Live, Indicators):
    ...
    def run_strategy(self, contracts, stop_1, target_1, trailing_1, stop_2, target_2, trailing_2, stop_3, target_3, trailing_3, periods, tempos, init, final):
        ...
        if self.operable:
            ...
            sent = False; first = False; second = False; third = False
            self.save_position()
                    ...
                    if self.position == 0 and self.global_position == 0:                # if there's not opened positions
                        ...
                        if not (self.weekday == 4 and pd.to_datetime(self.hour).time() > pd.to_datetime('16:00:00').time()):
                            ...
                            if not sent and prediction_1 > 0 and prediction_2 > 0 and prediction_3 > 0 and allow_entry:
                                ...
                                if price_buy_in_1 > 0 and price_buy_in_2 > 0 and price_buy_in_3 > 0 : sent = True
...
if __name__ == '__main__':
    ...
    live_octopus = LiveOctopus(symbol=symbol, bot_name='Octopus Longs (demo)', temp='1 min', port=port, client=client, real=False)
    
    init = ['2022-02-04', '2022-01-14', '2021-11-19']
    final = '2022-02-11'  #'2022-02-11'

    periods = ['close', 'SMA_21', 'SMA_89']
    #tempos = ['540', '720']          #['180', '240'] ['540', '720']
    tempos = [['60', '120'], ['180', '240'], ['540', '720']]
    live_octopus.run_strategy(contracts=6, stop_1=18, target_1=16, trailing_1=0.8, stop_2=60, target_2=60, trailing_2=0.5,
                          stop_3=68, target_3=68, trailing_3=0.9, periods=periods, tempos=tempos, init=init, final=final)
