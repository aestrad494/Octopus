...
class Live():
    def __init__(self, symbol, bot_name, temp, port, client, real=False):
        self.symbol = symbol
    ...
    def initialize_csv(self):
        ...
        if not path.exists('trades/total_executions_%s.csv'%self.symbol):
            mkdir('trades')
            initial_executions = pd.DataFrame(columns=['instrument','action','qty','price','time','commission','local_id', 'order_id'])
        ...
        if not path.exists('trades/total_trades_%s.csv'%self.symbol):
            initial_trades = pd.DataFrame(columns=['instrument','market_position','qty','entry_price','exit_price','entry_time',
    ...
    def get_arctic_data(self, start, end='', tempo='5S'):
        ...
        if end == '':
            ...
            data = library.read(self.symbol, date_range=DateRange(start='%s' % start, end='%s' % end)).data
    ...
    '''def get_ib_data(self):
        ib_data = pd.read_csv('/datos/kafka/ib_data/%s_ib_data.csv'%self.symbol, 
    ...
    '''def get_last_bar(self):
        last_bar = pd.read_csv('/datos/kafka/last_bar/%s_last_bar.csv'%self.symbol, 
                                index_col=[0], parse_dates=True)
        return last_bar'''

    def daily_results_positions(self):
            ...
            if not self.send_daily:
                ...
                if len(trades) > 0:
                    try:
                        ...
                        day_profit = trades.loc[init_date:final_date].profit.sum()
                    except: day_profit = 0
                message = '='*30+'\n' + self.bot_name
    ...
    def weekly_metrics(self):
        ...
        if self.weekday == 4 and pd.to_datetime(self.hour).time() >= pd.to_datetime('16:59:00').time() and not self.send_weekly:
            ...
            if len(trades) > 0:
                ...
                week_trades = trades.loc[monday_date:self.date].copy(True)
                if len(week_trades):
                    self.calculate_metrics(week_trades)
                    message = '='*30 + '\nWEEK METRICS %s %s (%s - %s)'%(self.bot_name,self.symbol,monday_date,self.date) + '\nTotal Trades: %d'%self.trades_total
    ...
    def _save_execution(self, operation, order_type='market'):
        execution = []
        try: iterator = util.tree(operation)['Trade']['fills'] if order_type == 'market' else operation
        except: self.ib.sleep(1); iterator = util.tree(operation)['Trade']['fills'] if order_type == 'market' else operation
        for trade in iterator:
            ...
            if symbol == self.symbol:
                ...
                for _ in range(int(qty)):
                    self.local_id += 1
                    execu = {'instrument':symbol, 'action':action, 'qty':1, 'price':price, 'time':time, 'commission':commission,
                        'order_id':order_id, 'local_id':self.local_id}
                    execution.append(execu)
        return execution
    
    def _save_trade(self, order, order_type='market', comment=''):
        ...
        '''
        execution = self._save_execution(order, order_type)
        ...
        if len(np.unique(self.opened.action)) > 1:
            ...
            for i in range(num_calcing):
                ...
                trade = {'instrument':entry_ex.instrument, 'market_position':action, 'qty':qty, 
    ...
    def market_order(self, action, qty, comment=''):
        '''Open Market Order
        ...
        profit, price, commission = self._save_trade(order=operation, comment=comment)
    ...
    def braket_market(self, action, qty, sl_range, tp_range, max_stop, entry_price=0, send_message=True, prints=True, save_image=True, send_image=True):
        price_in = 0; sl_price = 0; tp_price = 0; ord_sl = []; ord_tp = []; time_in = ''
        ...
        if allow_margin:
            ...
            if not price_in > 0:
                ...
                try:
                    ...
                    sl_price = price_in - mult*sl_range if entry_price == 0 else entry_price - mult*sl_range
                    tp_price = price_in + mult*tp_range if entry_price == 0 else entry_price + mult*tp_range
                    ...
                    if send_message: self.send_message_in(action=action, price_in=price_in, sl=sl_price, tp=tp_price, qty=int(qty), type='trades')
            ...
            self.close_opened_position(); self.cancel_pending_orders()
            self.ib.disconnect()
            self.send_telegram_message('%s %s\nThere is not enough Margin'%(self.bot_name, self.symbol))
        return price_in, sl_price, tp_price, time_in, comm_in, profit, ord_sl, ord_tp

    def exit_pending(self, order_to_close, in_action, qty, price_in, time_in, comm_in, comment, send_message=True, prints=True, save_image=True, send_image=True):
        mult = 1 if in_action == 'SELL' else -1
        self.position += mult*int(qty)
        self.save_position()
        price_out, comm_out, profit = self.check_pendings(order_to_close, what='values', comment=comment)
        time_out = str(pd.to_datetime('%s %s'%(self.date, self.hour)).tz_localize(str(get_localzone())).tz_convert('US/Eastern').tz_localize(None))
        if send_message: self.send_message_out(action=in_action, price_out=price_out, qty=int(qty), profit=profit, comm_in=comm_in, comm_out=comm_out, comment=comment)
    ...
    def exit_market(self, order_to_cancel, in_action, qty, price_in, time_in, comm_in, comment, send_message=True, prints=True, save_image=True, send_image=True):
    ...
    def check_pendings(self, order, what='fill', comment=''):
        ...
        if what == 'fill':
            ...
            profit, price, commission = self._save_trade(order=operation, order_type='pending', comment=comment)
            return(price, commission, profit)
    ...
    '''def send_telegram_message(self, message, type='trades'): #
        ...
        if type == 'trades':
            name = self.bot_name.split()[0].lower()
    ...
    def send_message_in(self, action, price_in, qty, sl=0, tp=0, type='trades'):
        ...
        '''
        if type == 'inv':
            msg_in = '%s \n%s Opened in %s \nPrice: %5.2f \nContracts: %d \nAt: %s' % (self.bot_name, action, self.symbol, price_in, qty, self.hour)
        else:
            msg_in = '%s \n%s Opened in %s \nPrice: %5.2f \nS/L: %5.2f \nT/P: %5.2f \nContracts: %d \nAt: %s' % (self.bot_name, action, self.symbol, price_in, sl, tp, qty, self.hour)
        self.send_telegram_message(msg_in, type=type)
    
    def send_message_out(self, action, price_out, qty, profit, comm_in, comm_out, comment='', type='trades'):
        ...
        if type == 'trades':
            msg_out = '%s \n%s Closed in %s (%s) \nPrice: %5.2f \nContracts: %d \nProfit(USD): %5.2f \nCommissions(USD): %5.2f \nAt: %s' % \
                    (self.bot_name, action, self.symbol, comment, price_out, qty, profit, (comm_in+comm_out),self.hour)
        else:
            msg_out = '%s \n%s Closed in %s \nPrice: %5.2f \nContracts: %d \nProfit(USD): %5.2f \nAt: %s' % \
                    (self.bot_name, action, self.symbol, price_out, qty, (profit-comm_in-comm_out), self.hour)
    ...
    '''def graph_trades(self):
        'Graph trades of the current session
        '
        if pd.to_datetime(self.hour).time() == pd.to_datetime('22:00:00').time():
            trades = self.total_trades.copy(True)
            trades.set_index('exit_time', inplace=True)
            trades.index = pd.to_datetime(trades.index)
            init_date = '%s 18:00:00' % str(datetime.now().date() - timedelta(days=1))
            final_date = '%s 17:00:00' % str(datetime.now().date())
            trades_day = trades.loc[init_date:final_date].reset_index()
            if len(trades_day) > 0:
                for i in range(len(trades_day)):
                    action = trades_day.iloc[i].market_position
                    price_in = trades_day.iloc[i].entry_price
                    price_out = trades_day.iloc[i].exit_price
                    time_in = str(trades_day.iloc[i].entry_time)
                    time_out = str(trades_day.iloc[i].exit_time)
                    self.trade_image(action, price_in, price_out, time_in, time_out)
                self.print('%s %s | Trades Images Done!'%(self.date, self.hour))
            else:
                self.print('%s %s | No trades Today'%(self.date, self.hour))'''
    ...
    def save_daily_profit(self, profit):
        '''Save daily in a CSV file
        '''
        pd.DataFrame({'daily_profit': profit}, index=[0]).to_csv('daily_profit_%s.csv'%self.symbol, index=False)
    
    def check_daily_profit(self):
        '''Check if daily profit csv exists
        '''
        if not path.exists('daily_profit_%s.csv'%self.symbol):
            pd.DataFrame({'daily_profit': 0}, index=[0]).to_csv('daily_profit_%s.csv'%self.symbol, index=False)
            daily_profit = 0
        else:
            daily_profit = pd.read_csv('daily_profit_%s.csv'%self.symbol).iloc[0].values[0]
        return daily_profit

    def save_weekly_profit(self, profit):
        '''Save weekly in a CSV file
        '''
        pd.DataFrame({'weekly_profit': profit}, index=[0]).to_csv('weekly_profit_%s.csv'%self.symbol, index=False)
    
    def check_weekly_profit(self):
        '''Check if weekly profit csv exists
        '''
        if not path.exists('weekly_profit_%s.csv'%self.symbol):
            pd.DataFrame({'weekly_profit': 0}, index=[0]).to_csv('weekly_profit_%s.csv'%self.symbol, index=False)
            weekly_profit = 0
        else:
            weekly_profit = pd.read_csv('weekly_profit_%s.csv'%self.symbol).iloc[0].values[0]
    ...
    def check_margins(self, qty, action, ord_type='market', price=0, max_stop=0):
        ...
        try: self.ib.sleep(2); order_data = util.tree(self.ib.whatIfOrder(self.contract, order))['OrderState']; print(order_data)
        except: self.ib.sleep(2); order_data = util.tree(self.ib.whatIfOrder(self.contract, order))['OrderState']; print(order_data)
        # Get margin values
